...
// Reading mode only makes sense with exactly one version chip showing (no
// stacked/columns choice to make, nothing to keep visually distinct) and
// only for an actual translation -- Hebrew/Greek has no plain sentence text
// to flow, just per-word interlinear tokens (see renderPanelBody).
// Reading mode only makes sense with nothing else to keep in view beside
// it -- with a second panel open there's always something else on screen,
// so the toggle (and the panel-link-mode one that takes its exact slot
// once that's true -- see .panels-linkable in styles.css) disappears
// instead of just disabling.
function singleReadableTranslation(panelState) {
  if (state.panels.length !== 1) return null;
...
function updateReadingModeControls(panelState) {
  ...
  const eligible = translation != null;
  // The single source of truth for whether this panel's own row shows the
  // reading-mode icon at all (see .bible-panel.reading-eligible in
  // styles.css) -- set here, not just at panel creation or inside
  // renderPanelBody, so a panel-count change alone (see
  // updatePanelCountControls, which calls this on every existing panel)
  // re-hides it the moment a second panel makes it ineligible, without
  // needing a full body re-render to notice.
  elements.panel.classList.toggle("reading-eligible", eligible);
  ...
  if (active) {
    const lang = translationLanguage(translation);
    ...
    elements.readingTranslationName.textContent = translationMeta(translation).label;
...
async function toggleReadingMode(panelState) {
  if (singleReadableTranslation(panelState) == null) return;
...
function updatePanelCountControls() {
  ...
  const twoSelected = desktop ? [2, 3, 4].includes(state.desktopPanelMode) : state.touchPanelCount !== 1;
  ...
  // Reading mode and panel-link mode share one slot in every panel's own
  // header (see .panel-reading-mode-control/.panel-link-mode-control in
  // styles.css) and are mutually exclusive by panel count alone -- reading
  // mode only while exactly one panel is open (see singleReadableTranslation),
  // link mode only once a second panel exists to link with. This is the one
  // place every panel-count change (add, remove, reset) already funnels
  // through, so it's what keeps every *existing* panel's own toggle in sync
  // too, not just a freshly created one.
  document.documentElement.classList.toggle("panels-linkable", state.panels.length >= 2);
...
function applyDesktopPanelWidths() {
  ...
  setAllDesktopPanelWidths(exactPanelFitWidth(state.desktopPanelMode));
}
...
// Collapses back to a single panel at Genesis 1:1 with just NIV and GAE,
// and clears every study-tool lookup/history alongside it -- but unlike
// an actual factory reset, this never touches localStorage wholesale
// (so font size, touch panel count, desktop panel mode, and copy
// selection mode all survive exactly as they were) and never clears the
// browser's own cache. Per explicit request: a quick way back to a known
// starting point for the *panels and lookups* specifically, not a wipe
// of every preference along with them.
function resetSite() {
...
function createPanelElement(panelState, shouldScroll = false) {
  ...
  readingModeToggleEl.addEventListener("click", () => toggleReadingMode(panelState));
  linkModeToggleEl.addEventListener("click", () => toggleLinkMode(panelState));
...
function addPanel({ suppressScroll = false } = {}) {
  ...
  // from the same book/chapter/verse and the same translation chips as
  // whatever was already at the far right. "Rightmost" means rightmost
  // *visible* one, though: a linked group collapsed down to a single
  // visible member via its own hide/show toggle (see
  // applyLinkedPartnersVisibility) still keeps its other members sitting
  // in this same array, just not on screen -- plain array order alone
  // could land on one of those hidden panels instead of whatever the
  ...
  const panelState = {
    ...
    // dimmedTranslations only makes sense alongside a matching
    // activeStudyTool (a study tool being active is *why* the rest sit
    // dimmed, see toggleTranslationChip) -- inherited alongside it below
    // rather than left null, this would otherwise carry a source panel's
    // own dimmed plain-text rows into a fresh panel with no study tool
    // active to justify them, rendering as unusually cramped dimmed text
    // instead of the full study-tool pane they're meant to sit beside.
    dimmedTranslations: source?.dimmedTranslations ? [...source.dimmedTranslations] : [...DEFAULT_DIMMED_TRANSLATIONS],
    originalLanguageHidden: source?.originalLanguageHidden ?? false,
    activeStudyTool: STUDY_TOOL_IDS.includes(source?.activeStudyTool) ? source.activeStudyTool : null,
    readingMode: false,
  };
  // TSK has no state of its own to carry over -- it always shows whatever
  // its own panel's book/chapter/verse currently is (see renderPanelBody),
  // already copied above. STR does, though (a specific Strong's entry with
  // no connection to any one verse), so the new panel's own STR instance
  // is primed to open on whatever entry the source's STR was actually
  // showing -- otherwise it would silently fall back to its own cold-start
  // default (Strong's H0001, see createEmbeddedStrongsTool) instead.
  if (panelState.activeStudyTool === "STR") {
    // Not selectedWord directly -- loadPanel (about to run, see
    // createPanelElement below) always starts a freshly created panel with
    // clearWordLookup, which would just erase it again before the STR
    // instance ever got a chance to read it as its own starting default.
    // renderPanelBody's own study-tool branch applies this pending word
    // once the instance actually exists and then clears it, a one-time
    // catch-up rather than an ongoing field.
    const sourceWord = source.studyToolInstances?.STR?.getWord();
    if (sourceWord) panelState.pendingStrWord = sourceWord;
  }
...
async function goToPassage(panelState, passage, { record = true, scrollToTop = true } = {}) {
  ...
  if (!syncingGroupNavigation) {
    ...
    if (partners.length) {
      ...
      try {
        await Promise.all(partners.map((partner) => goToPassage(partner, target, { record })));
      } finally {
...
function buildTranslationLinesInto(pane, panelState, verseNumber, texts, list, highlightedList, dimmedList, originalLanguageHidden) {
  ...
  list.forEach((translation) => {
    if (translation === "NOTE") {
      buildNoteTranslationLinesInto(pane, panelState, verseNumber, highlightedList, dimmedList, list.length === 1);
      return;
    }
    ...
    // With nothing else in this column to tell apart, the label naming
    // which version this is has nothing left to do -- drop it and let the
    // text itself start flush left, where the label used to sit.
    line.classList.toggle("translation-line--solo", list.length === 1);
...
function buildNoteTranslationLinesInto(pane, panelState, verseNumber, highlightedList, dimmedList, solo) {
  const line = document.createElement("div");
  ...
  line.classList.toggle("translation-line--solo", solo);
...
function renderPanelBody(panelState) {
  ...
  if (panelState.readingMode && !readingTranslation) panelState.readingMode = false;
  updateReadingModeControls(panelState);
  if (panelState.readingMode && readingTranslation) {
    renderReadingFlow(panelState, readingTranslation);
    updatePanelControls(panelState);
    return;
  }
...
function applyLinkedPartnersVisibility(groupId) {
  ...
  for (const panel of members) {
    ...
    elements.panel.hidden = hiding && !isInitiator;
...
function toggleLinkedPartnersVisibility(panelState) {
  ...
  applyLinkedPartnersVisibility(panelState.linkGroupId);
}
...
function clearLinkedPartnersHiding(panelState) {
  const elements = panelElements.get(panelState.id);
  if (elements) elements.panel.hidden = false;
...
function enterLinkPicking(sourcePanelState) {
  ...
  for (const [panelId, elements] of panelElements) {
    elements.linkOverlay.hidden = panelId === sourcePanelState.id;
  }
}
...
function exitLinkPicking() {
  ...
  for (const elements of panelElements.values()) elements.linkOverlay.hidden = true;
}
...
// Keeps the language picker and number field in sync with whichever word
// is currently loaded, so an arbitrary jump via the search button always
// continues from where the dialog actually is rather than whatever was
// last typed.
...
function expandMorphologyCode(code, lang) {
  if (lang === "he") return expandHebrewMorphology(code);
...
// clicking an interlinear word, or the word is Hebrew -- STEPBible's own
// Hebrew grammar tags use a different letter scheme this app doesn't
// decode yet (see export_interlinear.py), so they're never exported.
