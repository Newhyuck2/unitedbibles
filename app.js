...
function setupTouchReorder({ item, handle, container, itemClass, id, getOrder, onReorder }) {
  ...
  item.addEventListener("pointerdown", (event) => {
    ...
    const finish = (finishEvent) => {
      ...
      // `dragging` latches true the moment the 6px move threshold is
      // crossed even once, and a real finger wobbles past that on what's
      // meant as a plain tap far more often than a mouse ever would --
      // especially on a chip whose own click still does something (STR/TSK,
      // HEB/GRK: see toggleTranslationChip) now that the whole body, not
      // just the old ⠿ handle, starts this tracking. Suppressing that tap's
      // own click just because it wobbled mid-gesture, even though the
      // finger is back where it started by release, broke exactly that
      // click. Falling back to the release-time displacement (or an actual
      // hoverTarget, meaning a reorder is genuinely about to happen) instead
      // reads a wobble-and-return as the tap it actually was.
      const endDx = finishEvent.clientX - startX;
      const endDy = finishEvent.clientY - startY;
      const wasRealDrag = dragging && (Boolean(hoverTarget) || Math.hypot(endDx, endDy) >= 6);
      if (wasRealDrag) {
...
function setupCombobox({ input, menu, items, selectedValue, matches, onSelect, selectOnFocus = true }) {
  ...
  // Only the main panel's own book combo has this overlay (see index.html);
  // every other book/chapter/verse combo's querySelector comes back null,
  // so updateBookDisplay below is a harmless no-op there.
  const bookDisplay = input.closest(".book-combo")?.querySelector(".book-input-display") ?? null;
  const bookDisplayEn = bookDisplay?.querySelector(".book-display-en") ?? null;
  const bookDisplayKo = bookDisplay?.querySelector(".book-display-ko") ?? null;

  function updateBookDisplay(item) {
    if (!bookDisplay) return;
    bookDisplayEn.textContent = item?.en ?? "";
    bookDisplayKo.textContent = item?.ko ?? "";
  }
  ...
  function render(query = "") {
    ...
    for (const [index, item] of filtered.entries()) {
      ...
      if (comboKind === "book" && item.en != null && item.ko != null) {
        const en = document.createElement("span");
        en.className = "combo-option-en";
        en.textContent = item.en;
        const ko = document.createElement("span");
        ko.className = "combo-option-ko";
...
const THEME_COLOR_BY_THEME = { light: "#284f43", dark: "#15130f" };
...
function applyTheme() {
  ...
  // actual content attribute the way HTMLElement does, so [hidden]'s CSS
  // rule silently never matched -- toggleAttribute forces the real
  // attribute either way.
  themeToggleButton.querySelector(".theme-toggle-sun").toggleAttribute("hidden", isDark);
  themeToggleButton.querySelector(".theme-toggle-moon").toggleAttribute("hidden", !isDark);
  const themeColorMeta = document.querySelector('meta[name="theme-color"]');
  if (themeColorMeta) themeColorMeta.content = THEME_COLOR_BY_THEME[state.theme];
}
...
function renderSearchResults(query, matches, bookCounts, totalTranslationMatches, elapsedMs) {
  searchBookList.replaceChildren();
  searchResults.replaceChildren();
